...
class TestCompassOutput < Test::Unit::TestCase
  ...
  Compass.compiler.sass_files.each do |sass_file|
    ...
    define_method "test_#{test_name}_compile" do
      ...
      relative_pwd = Pathname.new(test_file_pwd).relative_path_from(Pathname.new("#{Dir.pwd}/tests")).to_s
      ...
      FileUtils.mkdir_p "#{Dir.pwd}/tests/#{base_pwd}"
      ...
      begin
        ...
        if !passed
          ...
          diff_pwd = "#{Dir.pwd}/tests/#{relative_pwd}.diff"
